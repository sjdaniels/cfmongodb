...
<cfscript>
	...
	*/
	...
	byID = dbCol.findById( url.personId );
	...
	kidAges = dbCol.distinct( "KIDS.AGE", collection );
	...
	dbCol.update( doc = {NAME = "Ima CF Dev", HAPPY = true}, query= {NAME = "Ima PHP dev"} );
	afterUpdate = dbCol.findById( person["_id"] );
	...
	dbCol.update( doc={NAME = "Ima CF Dev", HAPPY = true}, query= {NAME = "Ima PHP dev"}, applySet = false );
	afterUpdate = dbCol.findById( person["_id"] );
	...
	dbCol.ensureIndex( ["NAME"] );
	dbCol.ensureIndex( ["BIKE"] );
	dbCol.ensureIndex( ["KIDS.AGE"] );
	writeDump(var=dbCol.getIndexes(), label="Indexes", expand="false");
